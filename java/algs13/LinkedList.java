...
public class LinkedList<T> implements Iterable<T> {
    ...
    public void add(T item) {
        final Node<T> last = post.prev;
        final Node<T> x = new Node<>();
        x.item = item;
        x.next = post;
        x.prev = last;
        post.prev = x;
    ...
    private class LinkedListIterator implements ListIterator<T> {
        ...
        public void remove() {
            ocInc();
            if (lastAccessed == null) throw new Error();
            final Node<T> lastPrev = lastAccessed.prev;
            final Node<T> lastNext = lastAccessed.next;
        ...
        public void add(T item) {
            ocInc();
            final Node<T> first  = current.prev;
            final Node<T> middle = new Node<>();
            final Node<T> last   = current;
            middle.item = item;
            first. next = middle;
            middle.next = last;
            last.  prev = middle;
    ...
    public static <T> void printForward(String s, LinkedList<Integer> list, ListIterator<T> iterator) {
        ...
        while (iterator.hasNext())
            StdOut.printf ("[%d,%d] ", iterator.nextIndex (), iterator.next ());
        while (iterator.hasPrevious())
            StdOut.printf ("[%d,%d] ", iterator.previousIndex (), iterator.previous());
        StdOut.println ();
    }
