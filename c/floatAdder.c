...
unsigned int	getMantissa	(float f)
{
  unsigned int	mantissa	= *(unsigned int*)&f;
   ...
   if(getPowerOf2(f) == -127){
         return (mantissa & MANTISSA_MASK);
    }

  return((mantissa & MANTISSA_MASK) | MANTISSA_HIDDEN_BIT );
}
		...
		)
{
  ...
  if  (signF == signG)
  {
	...
	if (powerOf2F > powerOf2G){
            powerOf2 = powerOf2F;
            mantissa = mantissaF + (mantissaG >> (powerOf2F - powerOf2G));
            sign = signF;
            ...
            powerOf2 = powerOf2G;
            mantissa = mantissaG + (mantissaF >>  (powerOf2G - powerOf2F));
            sign = signG;
